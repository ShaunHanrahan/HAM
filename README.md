...
- **Broadcast frequencies.** The follow panel lists the satellite's transmitters (downlink, uplink, and beacon frequencies, plus mode), merged from two keyless sources and tagged by origin. One is the [SatNOGS DB](https://db.satnogs.org/), resolved by the satellite's `sat_id` rather than its NORAD id. That's more complete, and it also reaches entries SatNOGS stores without a NORAD id, including a lot of weather and imaging birds. The other is the [JE9PEL amateur-satellite list](https://www.ne.jp/asahi/hamradio/je9pel/). n2yo's own API doesn't expose frequencies, and not every satellite turns up in either source. The browser remembers whatever it last saw for each satellite, so following one you've tracked before shows its frequencies right away instead of waiting on the lookup; it still checks for updates in the background and quietly swaps in anything new.
- **About this satellite.** One click in the follow panel pulls up a Wikipedia photo and summary for whatever you're tracking. It's found by matching the NORAD id to Wikidata first, then falling back to a name search that walks the closest few matches until one turns out to be a real article, so catalog spellings like "NOAA 19" still land on NOAA-19.
- **Closest pass over you.** The follow panel has a slider showing when a satellite will pass overhead. Set it to "next pass" for the very next time it rises above you, or widen the window to hours or days ahead to find whichever pass comes closest, along with its distance and how high it climbs.
...
- **Works on phones.** On small screens the sidebar becomes a bottom sheet. Swipe or tap the grabber to expand it, and the whole thing scrolls as one list so you're not hunting through nested panes. Follow a satellite and its details lead the sheet automatically. Drag with one finger to rotate the globe, pinch with two to zoom, and the rotation speed eases off as you zoom in so it stays controllable instead of racing.
